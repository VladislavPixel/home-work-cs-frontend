import type { IDepthForwardIterator } from "../types/interfaces";
...
describe("Проверяю итератор, который реализует прямой обход дерева в глубину.", () => {
  test("Создаю экземпляр.", () => {
    ...
    const iterator: IDepthForwardIterator = new DepthForwardIterator(
      treeNode,
      1
    );
  ...
  test("Вызываю метод [Symbol.iterator].", () => {
    ...
    const iterator1: IDepthForwardIterator = new DepthForwardIterator(
      treeNode,
      2
    );
    const iterator2: IDepthForwardIterator = new DepthForwardIterator(
      treeNode,
      2
    );
  ...
  test("Вызываю метод next с пустым деревом в итераторе.", () => {
    const iterator: IDepthForwardIterator = new DepthForwardIterator(null, 0);
