import type { INode, ILinkedList, IIteratorLinkedList } from "../types/interfaces";
...
class LinkedList<T> implements ILinkedList<T> {
	...
	deleteFirst(): INode<T | T[]> {
		if (this.first === null) {
			throw new Error("LinkedList is Empty, operation `deleteFirst` is not supported!");
		}
	...
	deleteLast(): INode<T | T[]> {
		if (this.last === null) {
			throw new Error("LinkedList is Empty, operation `deleteLast` is not supported!");
		}
	...
	display(): void {
		if (this.#total === 0) {
			throw new Error("LinkedList is Empty, operation `display` is not supported!");
		}
