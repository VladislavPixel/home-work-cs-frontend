...
describe("Проверяю итератор, который реализует симметричный (центрированный) обход в глубину.", () => {
  test("Создаю экземпляр.", () => {
    ...
    const iterator: ITraversalIterator = new SymmetricalIterator(treeNode, 1);
  ...
  test("Вызываю метод [Symbol.iterator].", () => {
    ...
    const iterator1: ITraversalIterator = new SymmetricalIterator(treeNode, 2);
    const iterator2: ITraversalIterator = new SymmetricalIterator(treeNode, 2);

    expect(iterator1[Symbol.iterator]()).toEqual(iterator2);
  });

  test("Вызываю метод next с пустым деревом в итераторе.", () => {
    const iterator: ITraversalIterator = new SymmetricalIterator(null, 0);
  ...
  test("Вызываю метод next не с пустым деревом в итераторе.", () => {
    ...
    const iterator: ITraversalIterator = new SymmetricalIterator(treeNode, 5);
  ...
  test("Итерирую объект итератора.", () => {
    const nextMock = jest.spyOn(SymmetricalIterator.prototype, "next");
    ...
    const iterator: ITraversalIterator = new SymmetricalIterator(treeNode, 2);
