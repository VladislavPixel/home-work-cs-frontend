...
describe("Проверяю итератор, который реализует симметричный (центрированный) обход в глубину.", () => {
  test("Создаю экземпляр.", () => {
    ...
    const iterator: ITraversalIterator = new SymmetricalIterator(
      treeNode,
      1
    );
  ...
  test("Вызываю метод [Symbol.iterator].", () => {
    ...
    const iterator1: ITraversalIterator = new SymmetricalIterator(
      treeNode,
      2
    );
    const iterator2: ITraversalIterator = new SymmetricalIterator(
      treeNode,
      2
    );
