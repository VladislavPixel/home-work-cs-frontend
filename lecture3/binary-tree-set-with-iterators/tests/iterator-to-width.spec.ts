...
describe("Проверяю итератор, который реализует обход дерева в ширину.", () => {
  test("Создаю экземпляр.", () => {
    ...
    const iterator: ITraversalIterator = new IteratorToWidth(treeNode, 1);
  ...
  test("Вызываю метод [Symbol.iterator].", () => {
    ...
    const iterator1: ITraversalIterator = new IteratorToWidth(treeNode, 2);
    const iterator2: ITraversalIterator = new IteratorToWidth(treeNode, 2);
  ...
  test("Вызываю метод next не с пустым деревом в итераторе.", () => {
    ...
    const iterator: ITraversalIterator = new IteratorToWidth(treeNode, 5);
  ...
  test("Итерирую объект итератора.", () => {
    ...
    const iterator: ITraversalIterator = new IteratorToWidth(treeNode, 2);
