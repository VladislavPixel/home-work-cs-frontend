...
describe("Проверяю бинарное дерево: ", () => {
  test("Создаю экземпляр.", () => {
	...
	test("Вызываю методы возвращающие итераторы для различных порядков обхода дерева.", () => {
		...
		const iterator1: ITraversalIterator = new DepthForwardIterator({ value: -432, left: null, right: null }, 1);

		const iterator2: ITraversalIterator = new IteratorToWidth({ value: -432, left: null, right: null }, 1);

		const iterator3: ITraversalIterator = new SymmetricalIterator({ value: -432, left: null, right: null }, 1);

		const iterator4: ITraversalIterator = new ReverseIterator({ value: -432, left: null, right: null }, 1);

		expect(binaryTreeSet.getIteratorForBreadthTraversal()).toEqual(iterator2);
		expect(binaryTreeSet.getIteratorForDirectDepthTraversal()).toEqual(iterator1);
		expect(binaryTreeSet.getIteratorForSymmetricalTraversal()).toEqual(iterator3);
